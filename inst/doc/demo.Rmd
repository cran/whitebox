...
This section will cover optional arguments to `wbt_init()` (`exe_path`, `wd`, `verbose`, `compress_rasters`, and `max_procs`) and their corresponding options and helper functions.
...
#### `compress_rasters` argument

This argument sets the WhiteboxTools `compress_rasters` option. If `TRUE` raster outputs are compressed.

See also: `wbt_compress_rasters()`

#### `max_procs` argument

This argument sets the maximum number of processes for tools that run in parallel or partially parallelize. Default: `-1` uses all of the available cores.

See also: `wbt_max_procs()`
